# AI Builders Daily | 2026-06-26

今天最值得看的是 Swyx 的文字动态，全部内容已按中文优先的信息卡方式整理在站内。

- 生成时间：2026年6月26日 12:06
- 时区：Asia/Shanghai
- 候选内容：29
- 纯文字候选：14

## 重点文字动态

### 1. Swyx

- 标题：很多人正在准备下周的演讲（恭喜！）。
- 原帖地址：https://x.com/swyx/status/2069964772003770673
- 发布时间：2026年6月25日 10:03
- 当前热度：188 Likes / 14 Reposts / 20 Replies
- 命中标签：Coding Agents、Foundation Models、Product Ships
- 为什么重要：这条文字动态直接覆盖你的关注主题：Coding Agents、Foundation Models、Product Ships。
- 翻译来源：Google fallback
- 推荐语：Swyx 的最新观点聚焦在可执行的一线经验，命中主题 Coding Agents / Foundation Models / Product Ships：lots of folks prepping talks next week (congrats!). Some thoughts from RLing on thousands of hours of engineer- and r…

#### 中文正文

下周很多人都在准备演讲（恭喜！）。以下是RLing在数千小时面向工程师和研究人员的演讲经验总结：

- AI生成的SVG图片比AI生成的图片好。幻灯片里最多只能放4张AI生成的图片，不管你妈妈觉得它们有多漂亮（当然，如果你的演讲主题就是图像生成，那就另当别论了）。

- 言简意赅。与其讲5个没有具体例子的要点，不如用一个包含5个令人惊喜的应用案例的要点来概括。

- 把代码放在屏幕上。工程师喜欢看代码。尤其是当他们可以对一些无关紧要的细节吹毛求疵的时候。

- 别忘了娱乐。真正幽默风趣，或者善于讲述相关的轶事，比堆砌又一个要点更重要。

- 准备一个主题。每个演讲都应该有一张“如果你能记住本次演讲的一件事，那就是这个”的卡片。1）很多人都不用这个卡片。 2）很多人都不做计划。你拿到一张幻灯片，能不能好好利用？

- 准备一张核心幻灯片。你肯定参加过那种演讲，每个人都拿出手机拍照的场景。因为人们浏览图片的频率是视频的1000倍，所以如果你有一张能爆款的幻灯片，你的演讲/幻灯片就更有可能爆红。如果你用心去做，你的幻灯片爆红的几率会大大增加，而大多数人并没有这样做。还在苦苦思索？收集一些你喜欢的演讲案例，并根据自己的情况调整它们的格式。你的幻灯片遵循幂律——不要把5%的时间浪费在20张幻灯片上，而是把80%的时间集中在一张幻灯片上，剩下的时间都用来铺垫这张幻灯片。

- 你可能不需要幻灯片。在IDE里做现场演示，即兴发挥，邀请观众角色扮演，与观众互动，唱歌/表演，为氛围视频配音，这些我都见过。风险/投入越高，回报也越高。

- 演讲要引人入胜。没有视觉辅助或画面糟糕的演讲勉强还能听下去，但音频质量差的演讲就彻底失败了。要自信，声音洪亮，注意语调变化，并能唤起听众的情感。

- 设计情感旅程。开头要精彩，结尾也要精彩，中间要有一个高潮迭起、令人捧腹大笑或引出核心论点的时刻。其他一切都是为了铺垫。

- 数据驱动的演讲常常被低估。展示精美的图表和令人惊讶的权威数据。利用你的演讲权威性，从数据中推断出支持更广泛论点的结论。如果做得好，你会觉得这是我客观地分析你展示的数据后得出的显而易见的结论，而不是你强加给我的结论。

- 如何推销你的产品/公司而不显得推销过度：告诉我所有关于你所解决的问题我以前不知道应该知道的知识，这样你才能赢得我的信任，让我相信你是能够彻底解决问题的最佳人选。

- 积极观看大量的演讲。就像任何事情一样，你需要大量的练习，也需要不断探索，才能找到适合自己的风格/榜样。被动式学习是指看完演讲后不进行任何反思，只是机械地自动播放下一个视频。主动式学习是指看完演讲后尝试分析其优缺点。如果你想真正提升自己的演讲水平，可以思考一下你对某个著名演讲的感受，然后把演讲稿转录下来，仔细阅读文字，再与一个“普通”的演讲进行比较，并制定一些规则来指导自己改进。

感谢 @dexhorthy 今晚组织了 AIEWF 演讲者准备聚会。我们应该多组织一些这样的聚会……

<details><summary>英文原文</summary>

**lots of folks prepping talks next week (congrats!).**

lots of folks prepping talks next week (congrats!). Some thoughts from RLing on thousands of hours of engineer- and researcher- focused talks:

- AI generated svgs > AI generated imgs. MAXIMUM 4 ai slop images in your slides, I don't care how pretty your mom thinks they are (exception ofc if your talk is ABOUT imagegen)
- Be pointy. Better to have 1 message with 5 surprising applications, than 5 messages with no concrete examples.
- Put code on screen. Engineers like to see code. Especially if they can nitpick it to death over things that aren't the point.
- Don't forget to Entertain. Being actually funny, or good at telling relevant anecdotes, is more important than adding yet another bullet point.
- Have a Thesis. every talk gets ONE "if you remember one thing from this talk, it is this" card. 1) SO MANY PEOPLE DON'T USE IT. 2) SO MANY PEOPLE DON'T PLAN FOR IT. you get one. use it well?
- Have a Thesis Slide. you've been in that talk where everyone gets their phone out to take a photo of the slide. Because people see 1000x more images than videos, you are much more likely to have a viral talk/slide if you have a single viral slide. You are much more likely to have a viral slide if you TRY and most people do not TRY. Struggling? Collect examples from talks you like and adapt their format to your thing. Your slides have a power law - don't spend 5% of your time each on 20 slides, spend 80% of your time on 1 slide and have the rest build up to that 1 slide.
- You might not need slides. live demo in IDE, off the cuff rambles, pull audience member up to roleplay, do call and response with the audience, sing/perform, voice over vibe videos, I have seen it all. higher risk/effort, higher reward when done well.
- Be pleasant to listen to. Talks with bad/no visuals still can be listened to. Talks with bad audio are DOA. Be confident, project, remember vocal variation, evoke emotion.
- Design the emotional journey. Start strong, end strong, have a peak aha/laugh/thesis moment in the middle. Everything else is buildup.
- Data driven talks are underrated. Present pretty charts and surprising, authoritative numbers. Use the stage authority to infer from the data to support the broader thesis. Done well, it will feel like my obvious conclusion from objectively looking at the data you presented, not you feeding the conclusion to me.
- How to shill your product/company without feeling salesy: teach me everything I didn't know I should to know about the problems you solve, and then you shall have earned the right to convince me you are the guys to trust to solve it once and for all.
- Actively watch a lot of talks. like with anything you both need a lot of reps AND you need to explore to find the styles/role models that you shine in. Passive = no introspection after watching, just mindlessly autoplay next video. Active = trying to articulate why a talk was good/bad after watching. If you want to REALLY hone it, think about how you feel about a famous talk and then TRANSCRIBE the talks and read the words on the printed page, and then compare with a "normal" talk and define rules you will follow for yourself to improve.

thanks to @dexhorthy for organizing the AIEWF Speaker prep meetup tonight. we should actually do more of these....

</details>

### 2. Swyx

- 标题：本期内容非常精彩：- 为什么 Databricks 能胜过 Snowflake (!
- 原帖地址：https://x.com/swyx/status/2069864073202905501
- 发布时间：2026年6月25日 03:23
- 当前热度：52 Likes / 4 Reposts / 8 Replies
- 命中标签：Coding Agents
- 为什么重要：这条文字动态直接覆盖你的关注主题：Coding Agents。
- 翻译来源：Google fallback
- 推荐语：Swyx 的最新观点聚焦在可执行的一线经验，命中主题 Coding Agents：LOTS of alpha in this pod: - Why Databricks beat Snowflake (! a straight answer!) - Why everyone is building a metaha…

#### 中文正文

本期播客内容丰富：

- 为什么 Databricks 能胜过 Snowflake（直截了当地回答！）

- 为什么现在大家都在构建元架构

- 为什么 @neondatabase 如此有意义（@nikitabase 的梗简直让人哭笑不得）

- LTAP 如何解决我在 @braintrust 播客中与 @ankrgyl 讨论过的 HTAP 难题

- @MosaicML + DBRX 的现状

- 如何在市值 1750 亿美元的巨型企业中保持研发/创业文化

- 在迈向智能体云的竞赛中，数据库、操作系统还是……网络，哪个知识/经验更重要？

非常荣幸受邀参加 @Data_AI_Summit，采访两位业内顶尖人物，并有机会畅谈从 @bennstancil 的现代数据栈主题到 @alighodsi 精彩的主题演讲等方方面面。

<details><summary>英文原文</summary>

**LOTS of alpha in this pod: - Why Databricks beat Snowflake (!**

LOTS of alpha in this pod:

- Why Databricks beat Snowflake (! a straight answer!)
- Why everyone is building a metaharness now
- Why the @neondatabase made so much sense (so much @nikitabase glazing its not even funny)
- How LTAP solves the HTAP dream I discussed with @ankrgyl in our @braintrust pod
- What happened to @MosaicML + DBRX
- How to maintain research/startup culture in a $175B megacorp
- What's more important knowledge/experience in the race to the agent cloud: databases, operating systems, or.... networking!

very honored to be invited to @Data_AI_Summit to interview two of the top people in our industry and somehow be able to jam on everything from the @bennstancil modern data stack theme to @alighodsi's amazing keynote aura

</details>

### 3. Guillermo Rauch

- 标题：人工智能将带来前所未有的创业热潮。
- 原帖地址：https://x.com/rauchg/status/2070001110866354345
- 发布时间：2026年6月25日 12:28
- 当前热度：475 Likes / 27 Reposts / 79 Replies
- 命中标签：通用 AI Builder 动态
- 为什么重要：Guillermo Rauch 仍然是高信号来源，值得保留在每日观察列表里。
- 翻译来源：Google fallback
- 推荐语：Guillermo Rauch 的最新观点聚焦在可执行的一线经验：AI will bring forth an unprecedented surge in entrepreneurship. From 'solopreneurs,' to the revitalization of the sma…

#### 中文正文

人工智能将带来前所未有的创业热潮。

从个体经营者到中小企业的复兴，再到当今时代最大型企业的崛起……这一切都将由人工智能奠定基础。

<details><summary>英文原文</summary>

**AI will bring forth an unprecedented surge in entrepreneurship.**

AI will bring forth an unprecedented surge in entrepreneurship.

From 'solopreneurs,' to the revitalization of the small & medium business segment, to the emergence of the largest companies of our times… providing the foundation of it all.

</details>

### 4. Aaron Levie

- 标题：这次发布会有一些细节之处，这些细节在实践中非常重要。
- 原帖地址：https://x.com/levie/status/2069975251476422664
- 发布时间：2026年6月25日 10:45
- 当前热度：178 Likes / 18 Reposts / 37 Replies
- 命中标签：Coding Agents、Product Ships
- 为什么重要：这条文字动态直接覆盖你的关注主题：Coding Agents、Product Ships。
- 翻译来源：Google fallback
- 推荐语：Aaron Levie 的最新观点聚焦在可执行的一线经验，命中主题 Coding Agents / Product Ships：There are some subtleties in this launch that are very important in practice. This isn’t just you interacting with Cl…

#### 中文正文

这次发布有一些微妙之处，在实践中至关重要。这不仅仅是你通过 Slack 与 Claude 进行一对一互动。在这种情况下，Claude 扮演着同事的角色，任何用户都可以共享地与他互动。

我们已经看到一些智能体编码系统开始采用这种模式（例如 OpenClaw 和 Hermes），而将其应用于通用知识工作则进一步推动了这一理念的发展。

因此，这意味着这位智能体同事需要拥有自己的一套资源、工具和数据才能工作。这与你授予它访问你个人资源和工具的权限截然不同，因为智能体可能会意外地将这些信息分享给任何人。

相反，智能体需要像系统中的其他用户一样，你需要仔细考虑它应该拥有哪些访问权限，并确保这些信息可以安全地与该群体共享。

如果你能做到这一点，它将非常强大。例如，通过将 Claude Tag 连接到 Box，您可以让 Claude 访问企业销售资料，用于销售对话中的问题解答或生成 RFP；访问品牌指南和营销素材，用于创建营销活动；访问产品路线图资料和产品文档，供编码人员使用；访问法务团队所有成员均可访问的合同等等。

但这只是 Box 的一个示例。您也可以让它访问您的产品或客户分析数据、CRM 信息、代码库以及其他资源和代理，以便进行协同工作。

看到代理在未来工作模式的不断创新，真是令人振奋。

<details><summary>英文原文</summary>

**There are some subtleties in this launch that are very important in practice.**

There are some subtleties in this launch that are very important in practice. This isn’t just you interacting with Claude in a 1:1 format via Slack. In this case, Claude acts as a coworker that any user can tap into in a shared way.

We’ve already seen some agentic coding systems start to adopt this pattern (as well as OpenClaw and Hermes), and doing it for general purpose knowledge work continues to push the idea forward.

As a result, what this means is that this agentic coworker needs its own set of resources, access to tools, and data to work with. This is not the same as you giving it access to your personal resources and tools, because the agent then could accidentally then share those out with anyone. 

The agent needs to instead be like any other user in the system, and you need to be thoughtful about what it should have access to, and make sure its information that is safe to share with that group.

When you can pull that off, it’s quite powerful. For instance, by connecting to Claude Tag to Box, you could have Claude access corporate sales materials for questions in sales conversations or generating RFPs, brand guidelines and marketing assets for campaign creation, product roadmap materials and product documentation for coding agents to use, contracts that anyone in the legal team can access, and more.

But this is just the Box example. You can equally have it access your product or customer analytics data, CRM information, codebase, and other resources and agents that would make sense to work on in a collective manner. 

It’s awesome to see continued innovation in what the future of work may begin to look like with agents.

</details>

### 5. Dan Shipper

- 标题：我们是否正在加速迈向一个人工智能可以做人类能做的一切的未来？
- 原帖地址：https://x.com/danshipper/status/2069805581263847467
- 发布时间：2026年6月24日 23:31
- 当前热度：63 Likes / 3 Reposts / 8 Replies
- 命中标签：Coding Agents、Foundation Models
- 为什么重要：这条文字动态直接覆盖你的关注主题：Coding Agents、Foundation Models。
- 翻译来源：Google fallback
- 推荐语：Dan Shipper 的最新观点聚焦在可执行的一线经验，命中主题 Coding Agents / Foundation Models：Are we hurtling toward a future where AI can do everything humans can? Edwin Chen (@echen) believes we might be. He’s…

#### 中文正文

我们是否正飞速迈向一个人工智能可以包揽人类一切的未来？

Edwin Chen (@echen) 认为或许如此。他是 Surge AI 的首席执行官，该公司是为前沿实验室提供专家数据的最大供应商之一。Surge 在未接受任何外部融资的情况下，营收突破了 10 亿美元，这使得 Edwin 对人工智能发展的速度有着独特的见解。

我曾公开表示，人工智能自动化实际上创造了更多的人类工作。我也相信，尽管人工智能的发展呈指数级增长，但我们距离人工智能取代人类还有很长的路要走，远比我们想象的要远。

这就是我邀请 Edwin 做客 @every 的“AI & I”节目的原因。我们探讨了各种不同的未来愿景，并讨论了人类是否还能在宇宙中保持其独特的地位，以及这种地位究竟是什么。

我们深入探讨了以下内容：

• 如果 Chen 所设想的未来成为现实，他担心这会让人们停止努力。一个答案来自科幻作家特德·姜的一篇短篇小说：即使你知道你的决定无关紧要，也要表现得好像它们很重要。

• 人工智能或许很快就能将“赢得菲尔兹奖”这样模糊的目标付诸实践。但我认为，它做不到的是设定自己的目标——人工智能没有内在动力，没有探索的欲望，也没有改变主意的能力。

• 一个针对用户参与度优化的模型并不能提供最有价值的用户体验。埃德温用一个模型花了二十轮时间润色一封毫无意义的邮件，最后克劳德告诉他直接发送就行了。

• 为什么人工智能仍然不擅长写作：模型学会了破解它们训练所用的指标。埃德温的“海明威基准测试”发现，有些模型会在每句话里都输出一个比喻，这种过度索引导致糟糕的阅读体验。

对于任何想了解随着模型能力的提升，我们自身定位的人来说，这都是必看的。

请观看下方视频！

时间戳

1. 引言：00:00:54

2. Surge 作为“通用人工智能的摇篮”：00:01:49

3. 人工智能在新数学方面的能力揭示了人类成就的哪些方面：00:04:46

4. 在人工智能无所不能的时代，我们该如何保持动力：00:07:29

5. 为提升用户参与度而优化人工智能模型的陷阱：00:14:34

6. 使用数据集进行训练与使用环境进行训练：00:29:34

7. 个人数据的价值：00:35:09

8. 为什么模型不擅长写作：00:39:40

9. 陈的通用人工智能发展历程：00:42:00

<details><summary>英文原文</summary>

**Are we hurtling toward a future where AI can do everything humans can?**

Are we hurtling toward a future where AI can do everything humans can?

Edwin Chen (@echen) believes we might be. He’s the CEO of Surge AI, one of the largest providers of expert data for frontier labs. Surge passed over $1 billion in revenue without raising any outside capital, and that gives Edwin a unique perspective on how quickly AI progress is accelerating.

I’m on the record arguing that AI automation actually creates more human work. I also believe that even though AI progress is accelerating exponentially, we’re much farther away from AI replacing humans than it might seem.

That’s why I had Edwin on @every’s AI & I. We batted around different visions of the future, and discussed whether humanity will retain its unique place in the universe, and what that might be.

We get into:
• If Chen’s version of the future materializes, he’s worried it’ll make people stop trying. One answer comes from a short story by science fiction writer Ted Chiang: Behave as if your decisions matter, even when you know they don’t.
• AI may soon be able to take a nebulous goal like “win a Fields Medal” and execute. What it can’t do, I argue, is set its own goals—LLMs have no intrinsic motivation, no drive to explore, no ability to just change their mind.
• A model optimized for engagement doesn’t provide the most valuable user experience. Edwin spent 20 rounds polishing a pointless email with one model before Claude told him to just send it.
• Why AI is still bad at writing: models learn to hack the metrics they're trained on. Edwin's Hemingway Bench found models outputting a metaphor in every single sentence, an overindexxing that makes for a terrible reading experience.

This is a must-watch for anyone interested in where we fit as models get more capable.

Watch below!

Timestamps
1. Introduction: 00:00:54
2. Surge as a "school for AGI": 00:01:49
3. What AI's capacity for novel mathematics says about human achievement: 00:04:46
4. Motivation in an era when AI can do everything: 00:07:29
5. The trap of optimizing AI models for engagement: 00:14:34
6. Training using datasets versus training using environments: 00:29:34
7. The value of personal data: 00:35:09
8. Why models are bad at writing: 00:39:40
9. Chen's AGI timeline: 00:42:00

</details>

### 6. Garry Tan

- 标题：我们在旧金山不做这种垃圾事，因为我们看到了这种残忍行为的后果……
- 原帖地址：https://x.com/garrytan/status/2069806049847321020
- 发布时间：2026年6月24日 23:33
- 当前热度：1718 Likes / 132 Reposts / 139 Replies
- 命中标签：Coding Agents
- 为什么重要：这条文字动态直接覆盖你的关注主题：Coding Agents。
- 翻译来源：Google fallback
- 推荐语：Garry Tan 的最新观点聚焦在可执行的一线经验，命中主题 Coding Agents：We aren’t doing this trash in San Francisco because we saw what these kinds of ruthless politicians do to the citizen…

#### 中文正文

我们在旧金山不搞这些破事，因为我们亲眼目睹了这些冷酷无情的政客是如何对待民众的。

亚裔老人被杀害

公共教育体系被瓦解

如果你开始清点他们的尸体，揭露他们的失职，极左势力就赢不了。https://t.co/WZnllDa30D

<details><summary>英文原文</summary>

**We aren’t doing this trash in San Francisco because we saw what these kinds of ruth…**

We aren’t doing this trash in San Francisco because we saw what these kinds of ruthless politicians do to the citizens

Asian elders killed
Public education dismantled

If you start counting their bodies and talking about their abdication of duty, the hard left can’t win https://t.co/WZnllDa30D

</details>

### 7. Garry Tan

- 标题：旧金山市检察官大卫·邱表示，向记者发送一份机密的法律文件是……
- 原帖地址：https://x.com/garrytan/status/2069934356899246083
- 发布时间：2026年6月25日 08:03
- 当前热度：95 Likes / 15 Reposts / 5 Replies
- 命中标签：Coding Agents
- 为什么重要：这条文字动态直接覆盖你的关注主题：Coding Agents。
- 翻译来源：Google fallback
- 推荐语：Garry Tan 的最新观点聚焦在可执行的一线经验，命中主题 Coding Agents：SF City Attorney David Chiu says sending a confidential legal brief to reporters is grounds for removal from public o…

#### 中文正文

旧金山市检察官邱信福表示，向记者发送机密法律文件是构成免职的理由。

杰基·菲尔德应该辞职。https://t.co/12qcV4Lhka

<details><summary>英文原文</summary>

**SF City Attorney David Chiu says sending a confidential legal brief to reporters is…**

SF City Attorney David Chiu says sending a confidential legal brief to reporters is grounds for removal from public office.

Jackie Fielder should resign. https://t.co/12qcV4Lhka

</details>

### 8. Peter Yang

- 标题：Claude Design 非常棒。
- 原帖地址：https://x.com/petergyang/status/2069992268963135897
- 发布时间：2026年6月25日 11:53
- 当前热度：53 Likes / 1 Reposts / 15 Replies
- 命中标签：Coding Agents
- 为什么重要：这条文字动态直接覆盖你的关注主题：Coding Agents。
- 翻译来源：Google fallback
- 推荐语：Peter Yang 的最新观点聚焦在可执行的一线经验，命中主题 Coding Agents：Claude Design is pretty great. I gave it a repo for a mobile app I'm building and it reproduced the screens perfectly…

#### 中文正文

Claude Design 真不错。

我给它提供了一个我正在开发的移动应用的仓库，它完美地复现了所有界面。

不过，它提示我保存令牌之后就停了😅 https://t.co/k6hQ53zmFN

<details><summary>英文原文</summary>

**Claude Design is pretty great.**

Claude Design is pretty great. 

I gave it a repo for a mobile app I'm building and it reproduced the screens perfectly.

Except after one prompt it's telling me to save tokens already 😅 https://t.co/k6hQ53zmFN

</details>

